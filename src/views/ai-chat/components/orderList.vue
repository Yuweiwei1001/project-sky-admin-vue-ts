<template>
  <div>
    <div class="container homecon">
      <h2 v-if="!isEmbedded" class="homeTitle homeTitleBtn">
        订单信息
        <ul class="conTab">
          <li
      ...
      <!-- 嵌入模式的标题插槽 -->
      <slot name="header">
        <h4 v-if="isEmbedded && title" class="embedded-title">{{ title }}</h4>
      </slot>
      <div class="">
...
export default class extends Vue {
  ...
  // 新增props支持嵌入模式
  @Prop({ default: false }) isEmbedded!: boolean  // 是否为嵌入模式
  @Prop({ default: null }) externalData!: any[]   // 外部传入的订单数据
  @Prop({ default: '' }) title!: string           // 标题
  @Prop({ default: false }) hideOperations!: boolean // 是否隐藏操作按钮
  @Prop({ default: false }) hidePagination!: boolean // 是否隐藏分页
