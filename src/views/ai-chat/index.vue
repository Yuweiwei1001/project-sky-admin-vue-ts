            ...
            <div class="message-content">
              <div class="message-bubble ai-bubble">
                ...
                <div v-else>
                  ...
                  <div v-if="!message.responseType || message.responseType === 'text'" class="text-content">{{ message.content }}</div>
    ...
    >
      <el-scrollbar style="height: 100%">
        <div class="order-top">
          <div>
            ...
            >
              {{ getOrderStatusText(dialogOrderStatus) }}
            </div>
          </div>
          <p><label>下单时间：</label>{{ orderDetailData.orderTime }}</p>
        </div>
        ...
        <div class="order-middle">
          <div class="user-info">
              ...
              >
                ...
                <span>{{
                  dialogOrderStatus === 5
                    ...
                    : orderDetailData.estimatedDeliveryTime
                }}</span>
              </div>
            ...
            >
              ...
              <span>{{
                dialogOrderStatus === 6
                  ? orderDetailData.cancelReason || orderDetailData.rejectionReason
          ...
          <div class="dish-info">
            ...
            <div class="dish-list">
              <div
                v-for="(item, index) in orderDetailData.orderDetailList"
                :key="index"
                class="dish-item"
              >
            ...
            <div class="dish-all-amount">
              <label>菜品小计</label>
              <span>￥{{
                orderDetailData.amount && orderDetailData.packAmount
                  ? (orderDetailData.amount - 6 - orderDetailData.packAmount).toFixed(2)
                  : '0.00'
              }}</span>
            </div>
          </div>
            ...
            <div class="amount-list">
              <div class="dish-amount">
                ...
                <span class="amount-price">￥{{
                  orderDetailData.amount && orderDetailData.packAmount
                    ? ((orderDetailData.amount - 6 - orderDetailData.packAmount).toFixed(2) * 100) / 100
                    : '0.00'
                }}</span>
              </div>
              ...
              <div class="package-amount">
                ...
                <span class="amount-price">￥{{
                  orderDetailData.packAmount
                    ? (orderDetailData.packAmount.toFixed(2) * 100) / 100
                    : '0.00'
                }}</span>
              </div>
              <div class="all-amount">
                ...
                <span class="amount-price">￥{{
                  orderDetailData.amount
                    ? (orderDetailData.amount.toFixed(2) * 100) / 100
                    : '0.00'
                }}</span>
              </div>
              <div class="pay-type">
                ...
                <span class="pay-value">{{
                  orderDetailData.payMethod === 1 ? '微信支付' : '支付宝支付'
                }}</span>
              </div>
              <div class="pay-time">
                <span class="pay-name">支付时间：</span>
                <span class="pay-value">{{ orderDetailData.checkoutTime }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <span v-if="dialogOrderStatus !== 6" slot="footer" class="dialog-footer">
        <el-button
          ...
          @click="handleRejectOrderFromDetail"
        >拒 单</el-button>
        <el-button
          v-if="dialogOrderStatus === 2"
          type="primary"
          @click="handleAcceptOrderFromDetail"
        >接 单</el-button>
...
import { orderAccept, orderReject, queryOrderDetailById } from '@/api/order'
...
export default class AiChat extends Vue {
  ...
  private dialogOrderStatus: number = 0
  private currentOrderRow: any = {}
  ...
  // 处理查看订单详情 - 参考orderList的goDetail方法
  private async handleViewOrderDetail(row: any) {
    try {
      ...
      // 调用API获取订单详情
      const { data } = await queryOrderDetailById({ orderId: row.id })
  ...
  // 处理接单操作
  private async handleAcceptOrder(row: any) {
    try {
      this.$message({
        type: 'info',
        message: '正在处理接单...'
      ...
      const response = await orderAccept({ id: row.id })
      ...
      if (response.data.code === 1) {
        ...
        this.$message({
          type: 'error',
          message: response.data.msg || '接单失败'
        })
      }
    } catch (error) {
      ...
      this.$message({
        type: 'error',
        message: '接单操作失败，请稍后重试'
      })
    }
  }

  // 处理拒单操作
  private async handleRejectOrder(row: any) {
    try {
      const { value: rejectionReason } = await this.$prompt('请输入拒单原因:', '拒单确认', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
      ...
      if (response.data.code === 1) {
        ...
        this.$message({
          type: 'error',
          message: response.data.msg || '拒单失败'
        })
      }
    } catch (error) {
      if (error === 'cancel') {
      ...
      console.error('拒单操作失败:', error)
      this.$message({
        type: 'error',
        message: '拒单操作失败，请稍后重试'
      })
    }
  }
  ...
  private refreshOrderData() {
    // 找到最后一条包含订单表格的消息并重新获取数据
    const lastTableMessage = this.messages.slice().reverse().find(msg =>
      msg.type === 'ai' && msg.responseType === 'table' && msg.tableData
    )

    if (lastTableMessage) {
      // 这里可以重新发送查询请求来刷新数据
      // 为了简化，我们显示一个提示让用户手动刷新
      this.$message({
        type: 'info',
        message: '操作完成，请重新查询订单获取最新状态'
      })
    }
  }
  ...
  // 关闭详情弹窗 - 参考orderList的handleClose方法
  private handleCloseDetail() {
    this.dialogVisible = false
  }

  // 从详情弹窗接单
  private async handleAcceptOrderFromDetail() {
    await this.handleAcceptOrder(this.currentOrderRow)
    this.dialogVisible = false
  }

  // 从详情弹窗拒单
  private async handleRejectOrderFromDetail() {
    await this.handleRejectOrder(this.currentOrderRow)
    this.dialogVisible = false
  }
}
</script>
